...
void ArchiveConverter::toOutputArchive(const std::string& in_path, const std::string& out_path) {


    WeightsDict =  torch::pickle_load(GetTheBytes(in_path)).toGenericDict(); 
    
    torch::serialize::OutputArchive archive;
    std::map<std::string, torch::serialize::OutputArchive> nestedArchives;

    for (auto& item : WeightsDict) {
        std::string key = item.key().toStringRef();
        std::vector<std::string> tokens;
        std::stringstream ss(key);
        std::string token;

        // Split the key by '.' and store the parts in tokens
        while (std::getline(ss, token, '.')) {
            tokens.push_back(token);
        }

        // Handle the nested structure
        torch::serialize::OutputArchive* currentArchive = &archive;
        for (size_t i = 0; i < tokens.size() - 1; ++i) {
            std::string& part = tokens[i];

            // Check if this part of the key already has an archive
            if (nestedArchives.find(part) == nestedArchives.end()) {
                // Create a new archive for this part
                nestedArchives[part] = torch::serialize::OutputArchive();
            }

            // Add the nested archive to the current archive
            currentArchive->write(part, nestedArchives[part]);
            currentArchive = &nestedArchives[part];
        }

        // Write the tensor to the deepest nested archive
        currentArchive->write(tokens.back(), item.value().toTensor());
    }
    
    archive.save_to(out_path);
